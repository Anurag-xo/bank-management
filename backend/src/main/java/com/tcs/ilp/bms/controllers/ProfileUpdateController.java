...
public class ProfileUpdateController {
  ...
  @Autowired private ProfileUpdateRepository repository;
  ...
  public ResponseEntity<?> create(@RequestBody ProfileUpdate update) {
    ...
    if (customer != null && "HOLD".equalsIgnoreCase(customer.getStatus())) {
      return ResponseEntity.status(403)
          .body("Account is on HOLD. Profile updates are not permitted.");
    }
  ...
  public ResponseEntity<?> updateStatus(
    ...
    if (reviewedBy != null) {
      ...
      if (reviewer != null && "HOLD".equalsIgnoreCase(reviewer.getStatus())) {
        return ResponseEntity.status(403)
            .body("Your account is on HOLD. You cannot perform this action.");
      }
    }
    ...
    if ("approved".equalsIgnoreCase(status)) {
      ...
      ProfileUpdate update =
          updates.stream().filter(u -> u.getId().equals(id)).findFirst().orElse(null);
      if (update != null) {
        userRepository.updateProfile(
            update.getCustomerUsername(),
            update.getNewName(),
            update.getNewEmail(),
            update.getNewPhone(),
            update.getNewAddress());
      }
    }
